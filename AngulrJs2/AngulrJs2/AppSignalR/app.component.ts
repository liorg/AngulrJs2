    ...
    template: `
        ...
        <div class="flex-row">
            <task class="flex"
                ...
                [apiUrl]="'/tasks/long'"></task>
            <task class="flex"
                [eventName]="'shortTask.status'"
                [apiUrl]="'/tasks/short'"></task>
        </div>
     `
})
export class AppComponent implements OnInit {
    ...
    constructor(
        ...
        this.connectionState$ = this.channelService.connectionState$
            .map((state: ConnectionState) => { debugger; return ConnectionState[state]; });

        this.channelService.error$.subscribe(
            (error: any) => { debugger; console.warn(error); },
            (error: any) => { debugger; console.error("errors$ error", error); }
        );
